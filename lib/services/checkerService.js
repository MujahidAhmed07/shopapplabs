...
export async function checkInstagram(username) {
  ...
  // Strategy 1: Probe via public Instagram viewer (Picuki) with anti-bot guard
  try {
    const probeRes = await axios.get(`https://www.picuki.com/profile/${username}`, {
      headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' },
      timeout: 5000,
      validateStatus: () => true
    });
    const html = typeof probeRes.data === 'string' ? probeRes.data : '';
    const isBotChallenge = html.includes('Just a moment') || html.includes('Attention Required') || html.includes('Checking your browser');
    if (!isBotChallenge) {
      if (probeRes.status === 200 && html.includes('profile-name')) {
        const nameMatch = html.match(/<h1[^>]*class="profile-name"[^>]*>([\s\S]*?)<\/h1>/i);
        const fullName = nameMatch ? nameMatch[1].trim() : undefined;
        return { status: 'TAKEN', url: profileUrl, full_name: fullName };
      }
      if (probeRes.status === 404 || html.includes('profile-not-found')) {
  ...
  try {
    ...
    if (response.status === 200 && response.data?.data?.user) {
      ...
      return { status: 'TAKEN', url: profileUrl, full_name: userData.full_name, followers: userData.edge_followed_by?.count };
    } else if (response.status === 404) {
      return { status: 'AVAILABLE', url: profileUrl };
    }
  } catch (error) {}

  return { status: 'UNKNOWN', url: profileUrl, message: 'Instagram login required (Verify directly)' };
}
...
export async function checkTikTok(username) {
  ...
  try {
    ...
    if (oembedRes.status === 200 && oembedRes.data?.author_name) {
      return { status: 'TAKEN', url: profileUrl, full_name: oembedRes.data?.author_name };
    } else if ([400, 404].includes(oembedRes.status)) {
      return { status: 'AVAILABLE', url: profileUrl };
    } else {
      return { status: 'UNKNOWN', url: profileUrl, message: 'Verify directly on TikTok' };
    }
  } catch (error) {
    return { status: 'UNKNOWN', message: 'Verify directly on TikTok', url: profileUrl };
  }
}
...
export async function checkReddit(username) {
  ...
  try {
    ...
    if (res.status === 404 || html.includes('nobody on Reddit goes by that name') || html.includes('page not found') || html.includes('Sorry, nobody on Reddit')) {
      return { status: 'AVAILABLE', url: profileUrl };
    }
    if (html.includes('blocked by network security') || html.includes('Welcome to Reddit') || [403, 429].includes(res.status)) {
      return { status: 'UNKNOWN', url: profileUrl, message: 'Reddit security checkpoint (Verify directly)' };
    }
    if (res.status === 200 && (html.includes('karma') || html.includes('reddit-profile') || html.includes(`"name":"${username}"`))) {
      return { status: 'TAKEN', url: profileUrl };
    }
    return { status: 'UNKNOWN', url: profileUrl, message: 'Verify directly on Reddit' };
  } catch (error) {
    return { status: 'UNKNOWN', message: 'Verify directly on Reddit', url: profileUrl };
  }
}
...
export async function checkGitlab(username) {
  ...
  try {
    const [resUser, resGroup] = await Promise.all([
      axios.get(`https://gitlab.com/api/v4/users?username=${username}`, {
        headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)' },
        timeout: 6000,
        validateStatus: () => true
      }),
      axios.get(`https://gitlab.com/api/v4/groups/${username}`, {
        headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)' },
        timeout: 6000,
        validateStatus: () => true
      })
    ...
    const isUser = resUser.status === 200 && Array.isArray(resUser.data) && resUser.data.length > 0;
    const isGroup = resGroup.status === 200;

    if (isUser || isGroup) {
      const fullName = isUser ? resUser.data[0]?.name : resGroup.data?.name;
      return { status: 'TAKEN', url: profileUrl, full_name: fullName };
    }
    if (resUser.status === 200 && resGroup.status === 404) {
      return { status: 'AVAILABLE', url: profileUrl };
    }
    return { status: 'UNKNOWN', url: profileUrl };
  } catch (error) {
...
export async function checkPinterest(username) {
  ...
  try {
    const res = await axios.get(`https://www.pinterest.com/oembed.json?url=https://www.pinterest.com/${username}/`, {
      headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)' },
      timeout: 6000,
      validateStatus: () => true
    });
    if (res.status === 200 && res.data?.author_name) {
      return { status: 'TAKEN', url, full_name: res.data.author_name };
    }
    if ([400, 404].includes(res.status)) {
      return { status: 'AVAILABLE', url };
    }
    return { status: 'UNKNOWN', url, message: 'Verify directly on Pinterest' };
  } catch (error) {
    return { status: 'UNKNOWN', message: 'Verify directly on Pinterest', url };
  }
}
...
export async function checkSpotify(username) {
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      const titleMatch = html.match(/<title[^>]*>([\s\S]*?)<\/title>/i);
      const title = titleMatch ? titleMatch[1].trim() : '';
      return { status: 'TAKEN', url, full_name: title.replace(' on Spotify', '') };
    }
    if (res.status === 404) return { status: 'AVAILABLE', url };
    return { status: 'UNKNOWN', url, message: 'Verify directly on Spotify' };
  } catch (error) {
    return { status: 'UNKNOWN', message: 'Verify directly on Spotify', url };
  }
}
...
export async function checkMedium(username) {
  ...
  try {
    const res = await axios.get(`https://medium.com/feed/@${username}`, {
      headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)' },
      timeout: 6000,
      validateStatus: () => true
    });
    const content = typeof res.data === 'string' ? res.data : '';
    if (res.status === 200 && (content.includes('<rss') || content.includes('<channel>'))) {
      const titleMatch = content.match(/<title>([\s\S]*?)<\/title>/i);
      const title = titleMatch ? titleMatch[1].replace(/<!\[CDATA\[|\]\]>/g, '').replace('Stories by ', '').replace(' on Medium', '').trim() : undefined;
      return { status: 'TAKEN', url, full_name: title };
    }
    if (res.status === 404) {
      return { status: 'AVAILABLE', url };
    }
    return { status: 'UNKNOWN', url, message: 'Verify directly on Medium' };
  } catch (error) {
    return { status: 'UNKNOWN', message: 'Verify directly on Medium', url };
  }
}
...
export async function checkGenericProfile(username, urlPattern, profilePattern) {
  ...
  try {
    ...
    if (res.status === 403) {
      // Retry with clean UA to bypass bot fingerprint check (e.g. DeviantArt)
...
export async function checkDomain(domainName) {
  if (!domainName) {
    return { status: 'INVALID', url: '#', message: 'Domain name is required' };
  }

  // Domains NEVER contain underscores: sanitize any underscores to hyphens
  const cleanDomain = domainName.trim().toLowerCase().replace(/_/g, '-').replace(/-+/g, '-');
  const parts = cleanDomain.split('.');
  const namePart = parts[0];
  const tldPart = parts.slice(1).join('.');

  if (!namePart || !/^[a-z0-9]([a-z0-9\-]*[a-z0-9])?$/.test(namePart)) {
    return { status: 'INVALID', url: '#', message: 'Domains can only contain letters, numbers, and hyphens (no underscores or leading/trailing hyphens)' };
  }

  const namecheapUrl = `https://www.namecheap.com/domains/registration/results/?domain=${cleanDomain}`;
  const siteUrl = `https://${cleanDomain}`;

  // Strategy 1: Cloudflare DNS Status (Status 0 = NOERROR / Taken, Status 3 = NXDOMAIN / Available)
  try {
    const headers = { 'accept': 'application/dns-json' };
    const resA = await axios.get(`https://cloudflare-dns.com/dns-query?name=${cleanDomain}&type=A`, { headers, timeout: 4000 });
    if (resA.data?.Status === 0 || resA.data?.Answer?.length > 0) {
      return { status: 'TAKEN', url: siteUrl };
    }
    if (resA.data?.Status === 3) {
      return { status: 'AVAILABLE', url: namecheapUrl };
    }
  } catch (error) {}

  // Strategy 2: Nameserver delegation check
  try {
    const headers = { 'accept': 'application/dns-json' };
    const resNS = await axios.get(`https://cloudflare-dns.com/dns-query?name=${cleanDomain}&type=NS`, { headers, timeout: 4000 });
    if (resNS.data?.Status === 0 || resNS.data?.Answer?.length > 0) {
      return { status: 'TAKEN', url: siteUrl };
    }
    if (resNS.data?.Status === 3) {
      return { status: 'AVAILABLE', url: namecheapUrl };
    }
  } catch (error) {}

  // Strategy 3: Authoritative RDAP registry query (handles .ai, .io, and parked domains)
  try {
    const rdapRes = await axios.get(`https://rdap.org/domain/${cleanDomain}`, {
      timeout: 4000,
      validateStatus: () => true
    });
  ...
  return { status: 'UNKNOWN', url: namecheapUrl, message: 'DNS registry verification pending' };
}
  ...
  switch (platform) {
    ...
    default:
      if (GENERIC_PLATFORMS[platform]) {
        ...
        // Domains NEVER have underscores: convert any underscores to hyphens
        const domainLabel = username.replace(/_/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '') || username;
        result = await checkDomain(`${domainLabel}.${tld}`);
      } else {
